...
|---|---|---|
| **Omma by Spline** (`omma.build`) | AI prompt → interactive web experiences (3D + motion + UI + code). **Free plan: 50 code-generation credits/month, no card** (image/3D gen and custom domain are paid Pro $29/mo). **No MCP.** Use the free credits as a **section / idea generator**, then port the export into our pages via `shared/lib.css`. Detailed budget plan + 8 paste-ready prompts in `PROMPTS-MANUAL.md` § 7. | Section variants + new section ideas + the My Legal Basement / Garden Suite landing (still a stub). The most leveraged free-tier tool we've added — 50 credits/month renews. |
