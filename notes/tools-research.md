...
| **Omma** (`omma.build`) | Spline's AI canvas (launched 2026-03-24). Prompt → interactive 3D + UI + code, WebGPU-capable, deploys to web. **Free plan = 50 code-generation credits/month, no card** (image gen / 3D gen / custom domain need Pro $29/mo). Verified at omma.build/pricing on 2026-05-26. | **No MCP.** Sole interface is the canvas at omma.build. | Strategy: use the free 50 credits/month as a **section / idea generator** — text-only code requests cost least. 8-prompt budget plan + port-back template in `PROMPTS-MANUAL.md` § 7. | [Spline changelog – Meet Omma](https://updates.spline.design/changelog/meet-omma-create-3d-websites-and-apps-with-ai-agents.), [Omma pricing](https://omma.build/pricing) |
